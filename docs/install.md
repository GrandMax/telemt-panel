...
Скрипт `install.sh` в корне репозитория разворачивает MTProxy с Fake TLS за Traefik: один порт 443, маршрутизация по SNI. Образ telemt можно **собрать из исходников** (локально) или **скачать готовый** из репозитория `grandmax/telemt-pannel`.
